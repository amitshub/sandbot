...
# -------------------------------------------------------------------
def _validate_single_website_training_input(
    website_url: str,
    sitemap_url: str,
    files,
):
    ...
    if sitemap_url:
        raise HTTPException(
            status_code=400,
            detail="For single website training, sitemap_url is not allowed.",
        )

    if files:
        raise HTTPException(
            status_code=400,
            detail="For single website training, file upload is not allowed.",
        )
...
# ==========================================================
# Website training helpers
# ==========================================================
def _clean_training_url(value: str) -> str:
    """Normalize a URL for duplicate detection without changing scraper behavior."""
...
def _active_kb_urls_for_tenant(tenant_id: int) -> set:
    """Return URLs already present in editable KB so registry cannot hide a deleted KB entry."""
    urls = set()
    try:
        for doc in load_active_knowledge_documents(tenant_id) or []:
            url = _clean_training_url(doc.get("url") or doc.get("source_url") or "")
            if url:
                urls.add(url)
    except Exception:
        pass
...
def _scraped_doc_key_and_hash(tenant_id: int, doc: dict):
    doc = doc or {}
    doc_url = _clean_training_url(doc.get("url") or doc.get("source_url") or "")
    doc_text = (doc.get("text") or doc.get("content") or "").strip()
    ...
    hash_payload = {
        ...
        "page_type": doc.get("page_type") or doc.get("source_type") or "",
    }
    doc_hash = sha256_text(json.dumps(hash_payload, ensure_ascii=False, sort_keys=True))
    doc_key = f"tenant::{tenant_id}::url::{doc_url or doc_hash}"
    return doc_key, doc_hash, doc_url


def _train_scraped_documents_per_url(
    *,
    tenant_id: int,
    scraped_documents: list,
    crawl_type: str,
    content_type: str,
    all_new_chunks: list,
    ...
    """
    Train website docs page-by-page instead of one batch-level scrape_key.
    This prevents one already-done URL/batch from blocking missing KB entries.
    """
    scraped_documents = scraped_documents or []
    existing_kb_urls = _active_kb_urls_for_tenant(tenant_id)
    trained_docs_count = 0

    raw_scrape_file = PENDING_SCRAPED_DIR / f"scraped_raw_website_{tenant_id}.json"
    save_json(raw_scrape_file, scraped_documents)
    move_file_safely(raw_scrape_file, DONE_SCRAPED_DIR / raw_scrape_file.name)

    for doc in scraped_documents:
        doc_key, doc_hash, doc_url = _scraped_doc_key_and_hash(tenant_id, doc)
        source_label = doc_url or doc_key

        # If registry says done but editable KB no longer has this URL, rebuild it.
        if is_done(doc_key, doc_hash) and (not doc_url or doc_url in existing_kb_urls):
        ...
        mark_processing(
            doc_key,
            doc_hash,
            {"source_type": "website", "url": doc_url, "crawl_type": crawl_type},
        )

        doc_chunks = docs_to_chunks([doc], source_key=doc_key, source_hash=doc_hash)
        if not doc_chunks:
            mark_failed(
                doc_key,
                doc_hash,
                "No chunks created from this URL.",
                {"source_type": "website", "url": doc_url},
            )
            failed_sources.append({"source": source_label, "error": "No chunks created from this URL."})
            continue
        ...
        save_knowledge_documents(
            ...
            tags=["website", crawl_type or "single_page", "training"],
        )

        all_new_chunks.extend(doc_chunks)
        trained_docs_count += 1
        if doc_url:
            existing_kb_urls.add(doc_url)

        mark_done(
            doc_key,
            doc_hash,
            len(doc_chunks),
            {"documents": 1, "source_type": "website", "url": doc_url, "content_type": content_type},
        )
        processed_sources.append(source_label)
...
async def train_agent(
    ...
    """
    Existing synchronous training endpoint.
    Supports website URL, sitemap URL, and uploaded files.
    Fix: website scraping is trained per URL, not as one batch-level scrape_key.
    """
    website_url = (website_url or "").strip()
    sitemap_url = (sitemap_url or "").strip()
    ...
    # 1. Existing data/website_data.json support - unchanged
        ...
        try:
            ...
            if is_done(source_key, source_hash):
                ...
                if chunks:
                    ...
                    mark_failed(source_key, source_hash, "No chunks created", {"source_type": "website_json"})
                    failed_sources.append({"source": source_key, "error": "No chunks created"})
    ...
    if website_url or sitemap_url:
        scrape_label = f"tenant::{tenant_id}::scrape::{crawl_type}::{website_url or sitemap_url}"
        try:
            ...
            trained_count = _train_scraped_documents_per_url(
                ...
                crawl_type=crawl_type,
                content_type=content_type,
                all_new_chunks=all_new_chunks,
                skipped_sources=skipped_sources,
                processed_sources=processed_sources,
                failed_sources=failed_sources,
            )
            website_documents_count += trained_count

        except Exception as exc:
            error_file = FAILED_DIR / "scrape_error.txt"
            error_file.write_text(str(exc), encoding="utf-8")
            mark_failed(scrape_label, "unknown", str(exc), {"source_type": "scrape"})
            failed_sources.append({"source": scrape_label, "error": str(exc)})

    # 3. Uploaded files - unchanged
    for upload in files:
        ...
        try:
            ...
            mark_processing(source_key, source_hash, {"file_name": original_name, "source_type": "file"})
            ...
            if parsed_doc and parsed_doc.get("text"):
                ...
                if chunks:
                    ...
                    mark_failed(source_key, source_hash, "No chunks created", {"file_name": original_name, "source_type": "file"})
                    failed_sources.append({"source": original_name, "error": "No chunks created"})
            else:
                move_file_safely(pending_path, FAILED_DIR / file_name)
                mark_failed(source_key, source_hash, "No text extracted", {"file_name": original_name, "source_type": "file"})
                failed_sources.append({"source": original_name, "error": "No text extracted"})

        except Exception as exc:
            if pending_path.exists():
                move_file_safely(pending_path, FAILED_DIR / file_name)
            mark_failed(f"file::{file_name}", "unknown", str(exc), {"file_name": original_name, "source_type": "file"})
            failed_sources.append({"source": original_name, "error": str(exc)})

    # If everything was duplicate but already exists in KB, this can be a successful no-op.
    if not all_new_chunks and website_documents_count == 0 and uploaded_documents_count == 0 and not skipped_sources:
        raise HTTPException(status_code=400, detail="No readable KB/chunks were created from this training source.")
...
def _run_training_job(
    ...
    try:
        ...
        website_url = (website_url or "").strip()
        sitemap_url = (sitemap_url or "").strip()
        uploaded_files_payload = uploaded_files_payload or []
        ...
        if not all_new_chunks and website_documents_count == 0 and uploaded_documents_count == 0 and not skipped_sources:
            raise ValueError("No readable KB/chunks were created from this training source.")
...
# -------------------------------------------------------------------
# 3) REPLACE ONLY THE START ENDPOINT WITH THIS VERSION
# -------------------------------------------------------------------
...
#     # Save tenant-wise business rules before training starts.
#     # Chatbot will pick these values from tenant_agent_settings.
#     upsert_tenant_business_rules(
#         tenant_id=tenant_id,
#         business_type=business_type,
#         allowed_scope=allowed_scope,
...
def save_agent_config(req: AgentConfigRequest, current_user: dict = Depends(get_current_user)):
    ...
    allowed_scope = (req.allowed_scope or current_config["business"].get("allowed_scope") or "").strip()
    blocked_claims = (req.blocked_claims or current_config["business"].get("blocked_claims") or "").strip()
            ...
            cur.execute(
                ...
                INSERT INTO tenant_agent_settings
                    (tenant_id, business_name, industry, business_type, business_description,
                     allowed_scope, blocked_claims, greeting_message, starter_questions, system_prompt, restriction_rules,
                     support_hours, last_training_summary)
                VALUES
                    (%s, %s, %s, %s, %s, %s, %s, %s, CAST(%s AS JSON), %s, %s, CAST(%s AS JSON), CAST(%s AS JSON))
                ON DUPLICATE KEY UPDATE
