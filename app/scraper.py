...
def scrape_by_request(website_url: str, sitemap_url: str, crawl_type: str, content_type: str):
    ...
    Supported modes:
    - single_page / single_website / single_website_url: scrape only the provided website_url.
    - sitemap / full_website / website: collect URLs from sitemap/homepage discovery.

    Important behavior:
    - Single website URL training should create exactly one training document for the provided URL.
    - Multi-page website training may use sitemap/homepage discovery.
...
def scrape_single_page(url: str, content_type: str):
    """
    Scrape one URL as a real training document.

    Important for `single_page` training:
    - Do NOT return only because links/images were found.
    - Return the requests result only when it has enough readable text.
    - If requests text is empty/too small, try Selenium so JS/builder pages can still
      produce text, chunks, FAISS metadata, and Knowledge Base entries.
    - If Selenium fails, keep the best requests text only when it has some readable text.
    """
    ...
    try:
        ...
        request_text = (request_doc.get("text") or "").strip()

        if len(request_text) >= MIN_TEXT_LENGTH_FOR_REQUESTS:
            print(f"[SCRAPER] requests accepted for {url}: chars={len(request_text)}")
            return request_doc

        print(
            f"[SCRAPER] requests text too small for {url}: "
            f"chars={len(request_text)}; trying selenium"
        )
    ...
    try:
        selenium_doc = scrape_single_page_selenium(url, content_type)
        selenium_text = (selenium_doc.get("text") or "").strip()

        if selenium_text:
            print(f"[SCRAPER] selenium accepted for {url}: chars={len(selenium_text)}")
            return selenium_doc

        print(f"[SCRAPER] selenium returned empty text for {url}")

    except Exception as exc:
        print(f"[SCRAPER] selenium failed for {url}: {exc}")

    # Last safe fallback: if requests had some readable text, train that.
    # Do not return image/link-only docs because docs_to_chunks() needs text.
    if request_doc and (request_doc.get("text") or "").strip():
        return request_doc
