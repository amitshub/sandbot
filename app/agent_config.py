...
def get_agent_config(current_user: Dict[str, Any] = Depends(get_current_user)):
    ensure_tenant_agent_settings_schema()
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT * FROM tenant_agent_settings WHERE tenant_id=%s LIMIT 1", (tenant_id,))
            settings = cur.fetchone() or {}
    finally:
        conn.close()

    return {"success": True, "config": _build_config(tenant, settings, tenant_id)}
