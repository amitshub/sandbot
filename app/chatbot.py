...
# def is_product_detail_intent(message: str) -> bool:
#     value = (message or "").lower()
#     return any(x in value for x in [
#         "detail", "details", "specification", "specifications",
#         "feature", "features", "benefit", "benefits",
#         "use", "uses", "application", "applications",
#         "material", "grade", "tell me about", "explain",
...
#             elif is_product_detail_intent(message):
#                 answer = generate_answer_with_llm(
#                     message=message,
#                     context=clean_context,
#                     instruction="""
#                     Give detailed product information using only KB context.
#                     Explain product purpose, material, usage, benefits, and applications.
#                     Do not answer with only links or images.
#                     Keep response natural, professional, and informative.
#                     Add product/source link only at the end if available.
#                     Never mention metadata like title, tags, chunk, or priority.
#                     """
#                 )
#             elif contains_product_page_intent(message) and not is_product_detail_intent(message):
#                 answer = "Here are some product page images and related product references."
...
def clean_ai_reply(reply: str) -> str:
    ...
    remove_phrases = [
        ...
        "Based on the trained data,", "Based on the trained reference,",
    ]
    for phrase in remove_phrases:
        cleaned = cleaned.replace(phrase, "").strip()

    # Never expose internal KB/FAISS metadata to customers.
    metadata_patterns = [
        r"(?im)^\s*Knowledge label\s*:.*$",
        r"(?im)^\s*Source URL\s*:.*$",
        r"(?im)^\s*Source\s*:.*$",
        r"(?im)^\s*Tags\s*:.*$",
        r"(?im)^\s*Priority\s*:.*$",
        r"(?im)^\s*Chunk\s*:\s*\d+\s*/\s*\d+\s*\.?$",
        r"(?im)^\s*Page type\s*:.*$",
        r"(?im)^\s*Metadata\s*:.*$",
    ]
    ...
    # Remove old placeholder options that make the bot look fake.
    cleaned = re.sub(r"(?im)^\s*Product\s*\d+\s*$", "", cleaned)
    cleaned = re.sub(r"(?im)^\s*Option\s*\d+\s*$", "", cleaned)
    cleaned = re.sub(r"\n{3,}", "\n\n", cleaned).strip()
...
def wants_link_or_catalogue(message: str) -> bool:
    value = (message or "").lower()
    return _has_phrase(value, [
        "link", "website", "web site", "url", "catalog", "catalogue",
        "brochure", "page", "product page", "visit website", "open website",
        "send link", "share link",
    ])


def is_pricing_or_quote_question(message: str) -> bool:
    value = (message or "").lower()
    return _has_phrase(value, [
        "price", "pricing", "cost", "rate", "rates", "quotation", "quote",
        "estimate", "budget", "expensive", "cheap", "cheapest", "how much",
        "commercial quote", "project quote",
    ])


def is_sales_guidance_question(message: str) -> bool:
    value = (message or "").lower()
    return _has_phrase(value, [
        "option", "options", "which pipe", "which pipes", "which grade",
        "what kind", "what type", "what types", "what should i use",
        "what should we use", "should opt", "suitable", "recommend",
        "suggest", "for my house", "for home", "for commercial",
        "commercial", "residential", "industrial", "plumbing pipe",
        "plumbing pipes", "ss pipe", "ss pipes", "stainless steel pipe",
...
def should_use_sales_agent_first(intent: str, message: str) -> bool:
    """Route sales/product/support questions to the KB-grounded agent before old hardcoded replies.

    Kept intentionally narrow so multitenant customization, contact, image, welcome,
    and human handoff flows continue exactly as before.
    """
    if intent in {
        "greeting",
        "language_preference",
    ...
    return is_pricing_or_quote_question(message) or is_sales_guidance_question(message)
...
def save_and_return_chat_response(
    ...
    answer = safe_customer_answer(answer)
    assets = assets if isinstance(assets, dict) else empty_assets()
    ...
    return {
        ...
        "links": assets.get("links", []),
        "sources": assets.get("sources", []),
        "images_count": len(assets.get("images", []) or []),
        "links_count": len(assets.get("links", []) or []),
        "history_count": len(history),
        "debug": {
            "tenant_id": tenant_id,
            **debug,
...
def chat_with_agent(session_id: str, message: str, tenant_id, top_k: int = 5) -> Dict:
    ...
    # KB-grounded intelligent sales/support route.
    # This runs before old product overview/detail branches so the bot answers like
    # a human sales assistant instead of dumping links or weak generic lists.
    if should_use_sales_agent_first(intent, message):
        agent_answer, agent_assets, agent_debug = run_sales_support_agent_safe(
            tenant_id=tenant_id,
            session_id=session_id,
            message=message,
            top_k=max(top_k, 8),
        ...
        if agent_answer:
            # Do not push URLs/assets unless the customer explicitly asks for a link,
            # catalogue, brochure, page, or website. This keeps sales answers human.
            if not wants_link_or_catalogue(message):
                agent_assets = empty_assets()

            return save_and_return_chat_response(
                tenant_id=tenant_id,
                session_id=session_id,
                history=history,
                message=message,
                ...
                debug={
                    "intent": intent,
                    "sales_support_agent_used": True,
                    "sales_support_agent_first": True,
    ...
    # Product detail questions should be answered through the LLM using FAISS context.
    # Do this before image/page CTA handling so the bot does not reply with only links/images.
    if is_product_detail_intent(message) and not _has_phrase(message, ["image", "images", "photo", "photos", "picture", "pictures", "show me", "show some", "show more"]):
        try:
            answer = ask_groq(message, context, history, settings=settings)
        except Exception as exc:
            print("[PRODUCT DETAIL GROQ ERROR]", repr(exc))
            answer = ""

        if not answer:
            answer = fallback_answer(message, settings)

        product_assets = collect_assets_from_results(results, max_images=1, max_links=6)
        save_history(tenant_id, session_id, history, message, answer)
        return {
            "answer": answer,
            "session_id": session_id,
            "images": [],
            "links": product_assets.get("links", []),
            "sources": product_assets.get("sources", []),
            "images_count": 0,
            "links_count": product_assets.get("links_count", 0),
            "history_count": len(history),
            "debug": {
                "tenant_id": tenant_id,
                "intent": "product_detail_request",
                "faiss_results": len(results),
                "context_found": bool(context),
                "context_length": len(context),
                "top_score": results[0].get("score") if results else None,
