...
# def wants_similar_image_fallback(message: str) -> bool:
#     """
#     When customer asks for images of an item not clearly present in product data
#     (example: taps), do not dead-end. Share nearby product/fitting images from
#     the same tenant knowledge, but be honest that they are related/similar.
#     """
#     value = (message or "").lower()
#     return any(x in value for x in [
#         "tap", "taps", "faucet", "faucets", "sink", "basin",
#         "leak", "leakage", "kitchen", "bathroom",
#     ])


# def collect_any_tenant_product_images(tenant_id, max_images: int = 6, exclude_images: List[str] = None) -> Dict:
#     """
#     Last-resort image fallback. It still uses only this tenant's trained FAISS
#     metadata; it never fetches public web images and never invents product data.
#     """
#     broad_queries = [
#         "product page images catalogue fittings pipes taps faucets sink bathroom kitchen",
#         "products catalogue images pipe fittings connector valve elbow tap faucet",
#         "stainless steel pipes fittings product images catalogue",
#     ]
#     combined = []
#     seen_keys = set()
#     for q in broad_queries:
#         try:
#             for item in search_faiss(q, tenant_id=tenant_id, top_k=20) or []:
#                 key = item.get("chunk_id") or item.get("url") or item.get("title") or str(item)[:120]
#                 if key in seen_keys:
#                     continue
#                 seen_keys.add(key)
#                 combined.append(item)
#         except Exception as exc:
#             print("[SIMILAR IMAGE FALLBACK SEARCH ERROR]", repr(exc))
#     ranked = rank_results_for_images(combined, "product images fittings pipes taps sink bathroom")
#     return collect_assets_from_results(
#         ranked,
...
#         # Sales-bot UX rule:
#         # If the customer asks for images, do not dead-end only because the exact
#         # word (tap/faucet/etc.) is not present in product metadata. First try exact
#         # images above; if nothing is found, share ANY available product images from
#         # the SAME tenant's trained catalogue/chunks. This remains multi-tenant safe:
#         # no public web images, no cross-tenant images, no hardcoded company data.
#         if not assets.get("images"):
#             catalog_assets = collect_any_tenant_product_images(
#                 tenant_id=tenant_id,
#                 max_images=limit,
#                 exclude_images=exclude if is_more_image_followup(message) else [],
#             )
#             if catalog_assets.get("images"):
...
#                 if is_more_image_followup(message):
#                     answer = "I found a few more available product images from our trained catalogue."
#                 else:
#                     answer = f"I couldn’t find exact {label} images saved here, but I can still share some available product images from our trained catalogue."
...
#                 answer = f"I don’t have any usable product images saved in the trained data right now. I can check this with the {team}."
#             else:
#                 answer = f"I couldn’t find a clear matching image for that exact item in the trained data. I can check this with the {team}."
...
def build_safe_service_reply(settings: Dict, message: str, context: str = "") -> str:
    ...
    if is_out_of_scope_service(message):
        return (
            f"I don’t have confirmed information that {business_name} provides that service. "
            f"I can connect you with the {team} to confirm."
        )
    ...
    service_words = ["service", "services", "installation", "install", "maintenance", "repair", "on-site", "site visit"]
    if any(word in value for word in service_words):
        # If tenant is configured as product/manufacturer, do not imply services are available.
        if business_type in {"manufacturer", "product_seller", "ecommerce", "supplier", "industrial_supplier"}:
            if allowed_scope:
                return (
                    f"{business_name} mainly helps with {allowed_scope}. "
                    f"I don’t have confirmed information about installation, repair, maintenance, or on-site services. "
                    f"I can connect you with the {team} to confirm."
                )
            return (
                f"{business_name} mainly helps with product information and support. "
                f"I don’t have confirmed information about installation, repair, maintenance, or on-site services. "
                f"I can connect you with the {team} to confirm."
            )

        # If tenant is a service provider, still answer only if KB/rules support it.
        if blocked_claims and any(word in blocked_claims.lower() for word in service_words):
            return (
                f"I don’t have confirmed information that {business_name} provides that specific service. "
                f"I can connect you with the {team} to confirm."
            )

        return f"I can help with the confirmed services saved in our knowledge base. Please tell me which service you need."
