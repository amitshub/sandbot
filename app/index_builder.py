...
def _product_page_boost(item: Dict, query: str) -> float:
    """Boost product/catalog pages without hardcoding tenant product names."""
    if not _query_wants_product_or_catalog(query):
        return 0.0
    url = str(item.get("url") or "").lower()
    title = str(item.get("title") or "").lower()
    page_type = str(item.get("page_type") or "").lower()
    haystack = f"{url} {title} {page_type}"
    boost = 0.0
    if page_type in {"product_page", "catalog", "catalogue"}:
        boost += 0.18
    if any(x in haystack for x in ["product", "products", "catalog", "catalogue", "shop", "item", "model", "category"]):
        boost += 0.12
    if item.get("images"):
        boost += 0.04
    try:
        boost += min(0.10, max(0.0, float(item.get("priority") or 0)) / 100.0)
    except Exception:
        pass
